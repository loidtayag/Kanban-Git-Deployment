...
const Settings = (props: {
  ...
  offlineData: MutableRefObject<iBoard[]>;
  setSelectedBoard: (value: iBoard) => void;
  setBoardNames: (value: string[]) => void;
}) => {
  return (
    <Button
      style={{ backgroundColor: props.online ? "black" : "inherit" }}
      onClick={() => {
        if (props.online) {
          window.alert("Will now update local storage");
          localStorage.setItem(
            "boards",
            JSON.stringify(props.offlineData.current)
          );
          props.setSelectedBoard(props.offlineData.current[0]);
          props.setBoardNames(
            props.offlineData.current.map((board: iBoard) => board.name)
          );
          ...
          window.alert(
            "Will now show user with id 0 in database because there's no account feature"
          );
          fetch("/boards/0")
            .then((res) => res.json())
            .then((json) => {
    ...
    >
      <Img
        alt="Settings"
        src={props.online ? "/go_offline.SVG" : "/go_online.SVG"}
      />
    </Button>
  );
};
